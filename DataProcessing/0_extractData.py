...
class Participant:
	...
	def __init__( self, results_dir, participant_number, questionnaire_filename='PEQRs.csv', debug=False, timeout=20 ):
		...
		if self._debug: print('\nParticipant {} initialising...'.format(self.ID))
	...
	def create_storage_variables( self ):
		...
		fp_cols = ['S1E','S1','S2E','S2','S3E','S3','S4E','S4','S5E','S5','S6E','S6','S7E','S7','S8E','S8','S9E','S9']
		...
		for session in self.session_dirs:
			...
			for scorecard in [ 'GraspFailureScorecard.csv', 'ADLScorecard.csv' ]:
				...
				try: 
					...
					if self._debug and session in ['1','4','8','9']: print('\t\t### No ADL file in session {}'.format(session))
	...
	def process_grasp_data( self, remove_timeouts=True ):
		...
		self.speed_metrics_cl = self.contact_times_cl.apply( lambda x: [ 1 - ( ( y - 1 ) / ( 10 ) ) if isinstance(y,(float, int)) else y for y in x ] )
		self.speed_metrics_nl = self.contact_times_nl.apply( lambda x: [ 1 - ( ( y - 1 ) / ( 10 ) ) if isinstance(y,(float, int)) else y for y in x ] )
		...
		self.grasp_breakdown = pd.DataFrame()
		...
		for session in range( int( len( self.grasp_results_combined.columns ) / 3 ) ): # Combined performance averages across trial numbers and sessions (averaged between CL and NL cases, irrespective of eggs)
			...
			grasps = pd.DataFrame( self.grasp_results_combined.iloc[ :, _mis_col ].sum(), index=['grasps'], columns=[_col_title] )
			crushes = pd.DataFrame( self.grasp_results_combined.iloc[ :, _mis_col + 2 ].sum(), index=['crushes'], columns=[_col_title] )
			drops = pd.DataFrame( self.grasp_results_combined.iloc[ :, _mis_col + 1 ].sum(), index=['drops'], columns=[_col_title] )
			total_mean_calc = pd.DataFrame( _tmp_df[_col_title].mean(axis=0, skipna=True, numeric_only=True), index=["mean"], columns=[_col_title] )
			_tmp_df = pd.concat( [ _tmp_df, total_mean_calc, grasps, crushes, drops ], axis=0 )
		...
		for session in range( int( len( self.grasp_results_cl.columns ) / 4 ) ): # Extracts and processes grasp results for Cognitive loading condition
			...
			grasps = pd.DataFrame( self.grasp_results_cl.iloc[ :, _egg_col +1 ].sum(), index=['grasps'], columns=[_col_title] )
			crushes = pd.DataFrame( self.grasp_results_cl.iloc[ :, _egg_col + 3 ].sum(), index=['crushes'], columns=[_col_title] )
			drops = pd.DataFrame( self.grasp_results_cl.iloc[ :, _egg_col + 2 ].sum(), index=['drops'], columns=[_col_title] )

			

			_tmp_df = pd.concat( [ _tmp_df, total_mean_calc, light_mean_calc, heavy_mean_calc, grasps, crushes, drops ], axis=0 )
		...
		for session in range( int( len( self.grasp_results_nl.columns ) / 4 ) ): # Extracts and processes grasp results for Low loading condition
			...
			grasps = pd.DataFrame( self.grasp_results_nl.iloc[ :, _egg_col +1 ].sum(), index=['grasps'], columns=[_col_title] )
			crushes = pd.DataFrame( self.grasp_results_nl.iloc[ :, _egg_col + 3 ].sum(), index=['crushes'], columns=[_col_title] )
			drops = pd.DataFrame( self.grasp_results_nl.iloc[ :, _egg_col + 2 ].sum(), index=['drops'], columns=[_col_title] )
	...
	def process_adl_results( self ):
		...
		manual.columns = manual.columns.map( lambda x:  x[-1] )          # Rename DF columns to be compatible with future transforms
		platform.columns = platform.columns.map( lambda x:  x[-1] )      #( if this doesnt work maybe phrase a y in x thing)

		self.adl_ratios = platform.T / manual.T     # Compute ratios for all ADLs

		self.adl_ratios.index.name = None
		self.adl_ratios.reset_index( inplace=True )
		self.adl_ratios.insert(0, 'ID', self.ID)
		self.adl_ratios.rename(columns={ 'index' : 'Session'}, inplace=True)
			...
			for loading in [ 'CL', 'NL' ]:
				if loading == 'CL':
					_tmp_df_quant = { 'ID' : self.ID,                                               # Participant ID
								...
								'grasps' : self.grasp_metrics_cl.loc['grasps', col],
								'crushes' : self.grasp_metrics_cl.loc['crushes', col],
								'drops' : self.grasp_metrics_cl.loc['drops', col] }
					
					_tmp_dct_qual = utils.extract_questionnaire_rows(self.questionnaire_results_cl, 'Session Number', str( session + 1 ) ).iloc[ :, 4: ]
					_tlx = _tmp_dct_qual.iloc[:,0:6]
					_qres = _tmp_dct_qual.iloc[:,6:]
					_tmp_dct_qual['TLX totals'] = _tlx.sum(axis=1)
					_tmp_dct_qual['Q totals'] = _qres.sum(axis=1)
				...
				if loading == 'NL':
					_tmp_df_quant = { 'ID' : self.ID,
								...
								'grasps' : self.grasp_metrics_nl.loc['grasps', col],
								'crushes' : self.grasp_metrics_nl.loc['crushes', col],
								'drops' : self.grasp_metrics_nl.loc['drops', col] }
...
if __name__ == "__main__":
	...
	combined_adl_dataframe = pd.DataFrame()
	...
	for participant in range(len(participant_folders) ): # Creates a Participant object for each results folder and stores them in a list
		...
		if os.path.isdir(os.path.join( results_dir, participant_folders[participant] )): # Checks all paths to make sure they are a directory
			...
			if not _self.ID.endswith('OPP'):
				...
				combined_adl_dataframe = pd.concat( [ combined_adl_dataframe, _self.adl_ratios ], ignore_index=True )
				combined_analysis_dataframe = pd.concat( [combined_analysis_dataframe, _self.analysis_df]).drop('index', axis=1)
	
	# combined_adl_dataframe.reset_index( inplace=True, drop=True )
	combined_adl_dataframe = combined_adl_dataframe.join( combined_adl_dataframe['ID'].isin( feedback_mask ).rename('Feedback') ) # Adds in column containing boolean map of participants in the feedback condition
	combined_adl_dataframe.reset_index( inplace=True, drop=True )
	...
	combined_adl_dataframe.to_csv( os.path.join( os.getcwd(), os.path.join('Output logs', 'adl_df.csv')))
